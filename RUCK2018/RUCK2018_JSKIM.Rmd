...
```shell
# For Let's Encrypt
docker-machine ssh manager1 "DOMAINNAME=<YOUR DOMAIN> && \ 
                             mkdir /home/js/opt && \ 
...
## *.<YOUR DOMAIN> & <YOUR DOMAIN> should be in DNS "A or CNAME": digitalocean case.
[acme.dnsChallenge]
...
https://monitor.<YOUR DOMAIN> 에서 dashboard를 볼 수 있다.
...
docker service create \
    ...
    --network traefik-net \
     jinseob2kim/docker-rshiny
```
https://server.<YOUR DOMAIN> 에서 'rstudio server'를, https://app.<YOUR DOMAIN> 에서 'shiny server'를 실행할 수 있다. 
...
docker service create \
    ...
    nginx 
```
https://<YOUR DOMAIN>, https://www.<YOUR DOMAIN> 에서 [nginx](https://www.nginx.com/) 실행환경을 볼 수 있다. 
